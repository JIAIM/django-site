...
def reqister_email():
    ...
    return render_template('register_email.html', title='Registration')
...
def reqister(email):
    """ Реєєстарція почти після падтвердження """
    form = RegisterForm()
    if form.validate_on_submit():
...
def news():
    ...
    if request.method == 'POST':
        ...
        if 'all' in list(dict(data).keys()):
            ...
            url = ('https://newsapi.org/v2/top-headlines?country=ua&apiKey=0f0093970f924842925140bfbc1f7926')
...
def confirm_forgot_password(user_email):
    """ Підтвердження пароля для його зміни """
    if request.method == 'POST':
        if "back" in list(dict(request.form).keys()):
            return redirect('/login')
        if "return_code" in list(dict(request.form).keys()):
